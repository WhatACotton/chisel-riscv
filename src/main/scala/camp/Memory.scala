...
class Memory(
    memoryPath: Option[Int => String],
    baseAddress: UInt = "x80000000".U,
    sizeInBytes: Int = 16384
) extends Module {
  ...
  when(rvalid) {
    printf(
      p"Data read address=0x${Hexadecimal(dmemAddrReg)} data=0x${Hexadecimal(rdata)}\n"
    )
  }
  when(io.dmem.wen) {
    for (i <- 0 to 3) {
      when(io.dmem.wstrb(i)) {
        mems(i).write(
          (io.dmem.addr - baseAddress) >> 2,
          io.dmem.wdata(8 * (i + 1) - 1, 8 * i)
        )
      }
    }
  }
